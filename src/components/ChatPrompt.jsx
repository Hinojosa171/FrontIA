...
const ChatPrompt = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setIsLoading(true);
      const res = await axios.post(`${BACKEND_URL}/api/chat`, { prompt });
      setConversations([...conversations, { role: 'assistant', content: res.data.response }]);
    } catch (error) {
      console.error('Error al obtener respuesta:', error);
      setConversations([
        ...conversations,
        { role: 'system', content: `Error: ${error.response?.data?.error || 'No se pudo conectar al servidor'}` }
      ]);
    } finally {
      setIsLoading(false);
    }
